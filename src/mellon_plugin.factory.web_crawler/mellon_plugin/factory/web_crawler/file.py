...
from mellon_plugin.factory.web_crawler.web_crawler import pipelines
            ...
            try:
                ...
                if mellon.IBinaryChecker(item['response']).check():
                    yield component.createObject(\
                        u'mellon_plugin.factory.web_crawler.byte_file',
                        item, self.config)
                else:
                    yield component.createObject(\
                        u'mellon_plugin.factory.web_crawler.unicode_file',
                        item, self.config)
            except Empty:
                pass
            time.sleep(2)
